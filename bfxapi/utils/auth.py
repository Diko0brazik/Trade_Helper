...
def generate_auth_headers(API_KEY, API_SECRET, path, body):
  ...
  h = hmac.new(API_SECRET.encode(encoding='UTF-8'), signature.encode(encoding='UTF-8'), hashlib.sha384)
...
def _gen_signature(API_KEY, API_SECRET, nonce):
  ...
  secret = API_SECRET.encode(encoding='UTF-8')
  sig = hmac.new(secret, authMsg.encode(encoding='UTF-8'), hashlib.sha384).hexdigest()
